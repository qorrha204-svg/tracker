...
export async function signup(_prevState, formData) {
  ...
  // TEMP DIAGNOSTIC: dump raw env var content to find a bad character.
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
  const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "";
  const badCharsIn = (s) =>
    Array.from(s)
      .map((c, i) => [i, c.codePointAt(0)])
      .filter(([, code]) => code > 255)
      .map(([i, code]) => `${i}:${code}`)
      .join(",") || "none";
  return {
    error: `[ENV DUMP] urlLen:${url.length} keyLen:${key.length} urlBad:${badCharsIn(url)} keyBad:${badCharsIn(key)} urlStart:${JSON.stringify(url.slice(0, 40))} keyStart:${JSON.stringify(key.slice(0, 20))} keyEnd:${JSON.stringify(key.slice(-20))}`,
  };
