...
- **개요**: 전체 통계 카드, Status 분포, 사업부별 평균 진척률, 대표 의사결정 필요 항목 — 항목 클릭 시 과제 상세 모달이 그 자리에서 열림
- **과제**: 핵심과제·부서별 과제 통합 목록. 칸반 보드(드래그로 Status 이동) ↔ 테이블 뷰 전환, 필터(유형/사업부/Status), "+ 새 과제"와 기존 과제 클릭이 동일한 상세 모달로 연결
